        ...
        for element in json['results']:
            ...
            for t in element['classes']:
                if t['uri'] != 'http://www.w3.org/2002/07/owl#Thing':
                    if t['uri'].startswith('http://dbpedia.org/ontology/') or t['uri'].startswith(
                            'http://www.wikidata.org/entity/') or t['uri'].startswith('http://schema.org/'):
                        types.add(t['uri'])
            ...
            if 'type' in element:
                for t in element['type']:
                    if t != 'http://www.w3.org/2002/07/owl#Thing':
                        if t.startswith('http://dbpedia.org/ontology/') or t.startswith(
                                'http://www.wikidata.org/entity/') or t.startswith('http://schema.org/'):
                            types.add(t)
...
if __name__ == '__main__':
    ...
    # print(f'Entities from GoogleKG: {entities}')
    print("Entities from GoogleKG:")

    for ent in entities:
        print(ent)

    print(f'1st Entity from GoogleKG: {entities[0]}')
    ...
    print(f'1st Entity from DBpedia with remove_tags: {remove_tags(entities[0])}')
    ...
    for ent in entities:
        print(ent)
    print(f'1st Entity from wikidata: {entities[0]}')
